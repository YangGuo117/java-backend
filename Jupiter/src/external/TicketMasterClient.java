...
public class TicketMasterClient {
	...
	private static final String API_KEY = "GHSItNfd2NbLytB12MaMNaT2DzmfsmMI";
...
//		String query = String.format("apikey=%s&latlong=%s,%s&keyword=%s&radius=%s", API_KEY, lat, lon, keyword, DEFAULT_RADIUS);
		...
		try {
			...
			if (responseCode != 200) { // ignore unsuccess request
				return new ArrayList<>();
			}
			// buffer reader line by line, InputeStream: read as a whole
...
//	}
	private String getAddress(JSONObject event) throws JSONException {
		if (event.isNull("_embedded")) return "";
